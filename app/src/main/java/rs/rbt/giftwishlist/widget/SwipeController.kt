...
class SwipeController(buttonsActions: SwipeControllerActions) : Callback() {
    ...
    override fun onChildDraw(c: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float, dY: Float, actionState: Int, isCurrentlyActive: Boolean) {
        ...
        if (actionState == ACTION_STATE_SWIPE) {
            if (buttonShowedState !== ButtonsState.GONE) {
                if (buttonShowedState === ButtonsState.LEFT_VISIBLE) finalDX = Math.max(dX, BUTTON_WIDTH)
                if (buttonShowedState === ButtonsState.RIGHT_VISIBLE) finalDX = Math.min(dX, -BUTTON_WIDTH)
                super.onChildDraw(c, recyclerView, viewHolder, finalDX, dY, actionState, isCurrentlyActive)
            } else {
                setTouchListener(c, recyclerView, viewHolder, finalDX, dY, actionState, isCurrentlyActive)
            }
        }
